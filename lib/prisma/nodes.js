...
export async function updateNodes(nodes) {
   // console.log('node.updateNodes nodes', nodes);
   try {
      const updatedNodes = await prisma.$transaction(
         Object.keys(nodes).map(nodeID => {
            const { id, ...updateData } = nodes[nodeID];
      ...
      console.log('node.updateNodes updatedNodes', updatedNodes);
      const updatedNodesDict = Object.fromEntries(updatedNodes.map(node => [node.id, node]))
