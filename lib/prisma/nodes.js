...
export async function updateNodes(nodes) {
   try {
      ...
      //       const { id, ...updatedNode } = nodes[nodeID];
      //       console.log("node.updateNodes updating node", updatedNode)
      ...
      for (let i = 0; i < nodesArray.length; i += batch_size) {
         const batch = nodesArray.slice(i, i + batch_size);
         ...
         for (const node of updatedNodesBatch) {
            console.log("nodes.updateNodes: ", node)
