...
export async function createNode(nodeData) {
   try {
      ...
      await updateNodeFrequencyValues(-frequencyDelta)
...
async function updateNodeFrequencyValues(frequencyDelta) {
   const nodes = await prisma.node.findMany({
      orderBy: { idx: 'asc' }
   });
...
async function getWeightedRandomNode() {
   ...
   try {
      ...
      // If no matching node is found, return the first node
      if (!randNode) {
