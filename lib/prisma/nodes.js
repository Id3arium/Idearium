...
export async function getNextRandomNode(currNodeID) {
   try {
      ...
      console.log("getNextRandomNode:", randNode)
...
async function getWeightedRandomNode() {
   try {
      ...
      console.log("nodes.getWeightedRandomNode() node:", nodes[0]);

      const redistResult = await redistributeNodeFrequencies(-1, nodes[0].idx)
      
      return nodes[0];
      // return redistResult.node;
   } catch (error) {
      console.log("nodes.getWeightedRandomNode() error:", error);
      return { error };
   }
