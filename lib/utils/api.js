...
export async function fetchNextRandomNode(currNode) {
    ...
    if (randNodeData.node) {
        console.log('API.fetfetchNextRandomNode() randNodeData.node', randNodeData.node)
...
export async function fetchNodeById(nodeID) {
    ...
    if (nodeData.node) {
        console.log('API.fetchNode() node', nodeData.node)
...
export async function removeNodeInDB(nodeID) {
    ...
    const data = await res.json();
    console.log('API.removeNodeInDB nodes from database', data)
    return data.node;
}
...
export async function changeNodeFrequency(frequencyChange, nodeIdx) {
    ...
    const data = await res.json();
    console.log('API.changeNodeFrequency node updated', data)
    return data.node;
}
...
export async function resetNodeFrequencies() {
    ...
    const data = await res.json();
    console.log('API.resetNodeFrequencies node updated', data)
    return data.node;
}