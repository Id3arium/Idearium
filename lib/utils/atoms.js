...
export const addNodeAtom = atom(null, async (get, set, newNode) => {
	...
	console.log("addNodeAtom adding node from client", newNode)
	...
	try {
		...
		console.log("addNodeAtom adding node from server", createdNode)
...
export const removeNodeAtom = atom(null, async (get, set, node) => {
	const nodes = get(clientNodesAtom);
	set(removeFromNodeTimelineAtom, node.id);
	
	for (const nodeId in nodes) {
        if (nodes[nodeId].ranking > node.ranking) {
            nodes[nodeId].ranking--;
        }
    }
	...
	try {
		await API.removeNodeInDB(node.id);
		set(nodesAtom, { ...nodes } )
	} catch (error) {
...
export const updateNodeAtom = atom(null, async (get, set, updatedNode) => {
    const nodes = get(clientNodesAtom);
    try {
        set(nodesAtom, { ...nodes, [updatedNode.id]: updatedNode });
        console.log("updateNodeAtom: Node updated successfully", updatedNode);
    } catch (error) {
        console.error("updateNodeAtom: Failed to update node:", error);
    }
});
...
function getCurrentIndexAfterPruning(currentIndex, indexesToRemove) {
	...
	if (nodesRemovedBeforeCurrent > 0) {
		...
		const precedingIdxInPruned = Math.max(...indexesToRemove.filter((i) => i < currentIndex));
		newCurrentIndex = precedingIdxInPruned
	}
...
export const upDistributeFrequencyAtom = atom(null, (get, set, nodeID) => {
	...
	set(redistributeFrequenciesAtom, frequencyChange, nodeID)
})
...
export const redistributeFrequenciesAtom = atom(null, (get, set, frequencyChange, nodeID) => {
	...
	console.log("atoms.redistributeFrequenciesAtom: ", frequencyChange, nodeID)
